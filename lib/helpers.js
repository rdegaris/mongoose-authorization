...
function getAuthorizedFields(schema, options, action, doc) {
  ...
  return (
    _.chain(authLevels)
      ...
      // .filter(path => schema.pathType(path) !== 'adhocOrUndefined') // ensure fields are in schema // rdegaris: override for deep arrays
...
function sanitizeDocument(schema, options, doc) {
  ...
  // Mongoose Document.
  let innerDoc = doc._doc || doc;
  let newDoc = _.pick(innerDoc, authorizedFields);

  let deepDoc = {};
  _.each(authorizedFields, item => {
    if (item.indexOf(".") === -1) return; // Already in doc from _.pick above. Example "notes"
    const arrDeep = item.split(".");

    let found;
    found = _.get(innerDoc, item); // Will pickup 'name.first'
    if (!found) found = _.get(innerDoc[arrDeep[0]][0], arrDeep[1]); // Will pickup 'services.array'

    if (Array.isArray(found)) {
      deepDoc[arrDeep[0]] = [];
      deepDoc[arrDeep[0]][arrDeep[1]] = [];
      for (const d of innerDoc[arrDeep[0]]) {
        found = _.get(d, arrDeep[1]);
        //deepDoc[arrDeep[0]][arrDeep[1]].push(found);
        deepDoc[arrDeep[0]].push(found);
      }
    }
  });

  newDoc = Object.assign(newDoc, deepDoc);
...
function sanitizeDocumentList(schema, options, docs) {
  ...
  const filteredResult = _.chain(docList)
    .map(doc => {
      const upgradedOptions = _.isEmpty(schema.pathsWithPermissionedSchemas) ? options : _.merge({}, options, { authPayload: { originalDoc: doc } });
